...
function filtrarFilmesPorAno (ano) {
    return catalogoFilmes.find(filme => filme.anoLancamento === ano)
}

const filmeEncontrado = filtrarFilmesPorAno(1993)
...
function filtrarOrdenarProdutosPorPreco (maxPreco) {
    const produdosFiltrados = listaProdutos.filter(produto => produto.preco <= maxPreco)
    return produdosFiltrados
}

console.log(filtrarOrdenarProdutosPorPreco(70.00));
