...
const pessoa = {
    ...
    idade: 68,
    solteiro: false,
    hobbies: ['ir ao bar do Moe', 'bater no Bart']
}

function mostrarInfoPessoa (pessoa) {
    console.log(`O nome da pessoa é: ${pessoa.nome}`)
    console.log(`Sua idade é de: ${pessoa.idade}`)
    console.log(`Está solteiro?: ${pessoa.solteiro}`)
    console.log(`Tem os hobies de: ${pessoa.hobbies}`)
}
...
pessoa.endereco = {
    rua: 'Evergreen Terrace',
    cidade: 'Springfield',
    estado: 'um estado ao norte de Nevada'
}
...
console.log(filtrarPorCidade(pessoas, 'Springfield'))
...
const calculadora = {
    ...
    calcularMedia: function (numbers) {
        const soma = numbers.reduce((total, number) => total + number, 0);
...
const mediaNumbers = [2, 2, 2, 2, 2]
console.log(`A soma da média é: ${calculadora.calcularMedia(mediaNumbers)}`)
